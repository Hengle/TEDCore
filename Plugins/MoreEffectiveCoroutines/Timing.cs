...
namespace MEC
{
    public class Timing : MonoBehaviour
    {
        ...
        public static float WaitUntilDone(CoroutineHandle otherCoroutine, bool warnOnIssue)
        {
            ...
            if (warnOnIssue)
                TEDDebug.LogWarning("WaitUntilDone cannot hold: The coroutine handle that was passed in is invalid.\n" + otherCoroutine);
        ...
        private IEnumerator<float> WaitUntilDoneWrapper(IEnumerator<float> coptr, CoroutineHandle handle)
        {
            ...
            {
                if (warnOnIssue)
                    TEDDebug.LogWarning("A coroutine attempted to wait for itself.");
            ...
            {
                if (warnOnIssue)
                    TEDDebug.LogWarning("A coroutine attempted to wait for a coroutine running on a different MEC instance.");
